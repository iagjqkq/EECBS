...
class GridWorld(Environment):
    ...
    def __init__(self, params) -> None:
        ...
        params[ENV_ACTION_SPACE] = NR_GRID_ACTIONS
        ...
        self.dynamic_collision_weight = torch.tensor(0.0)  # 新增动态权重参数
        ...
        self.viewer = None
        self.agent_costs = self.int_zeros(self.nr_agents)
    ...
    def step(self, joint_action):
        ...
        min_tensor = torch.zeros_like(self.current_positions)
        max_tensor = torch.tensor([[self.rows-1, self.columns-1]], device=self.device).expand_as(self.current_positions)
        new_positions = (self.current_positions + joint_action).clamp(min=min_tensor, max=max_tensor)
    ...
    def is_terminated(self):
        ...
        # 新增目标状态追踪
        new_goal_reached = torch.logical_and(
            self.current_positions[:,0] == self.goal_positions[:,0],
            self.current_positions[:,1] == self.goal_positions[:,1]
        )
        # 核心修改：当智能体首次到达目标时，记录其成本
        not_yet_reached = torch.logical_not(self.actual_goal_reached)
        first_time_reached = torch.logical_and(new_goal_reached, not_yet_reached)
        self.agent_costs[first_time_reached] = self.time_step
        # 合并历史达成状态
        self.actual_goal_reached = torch.logical_or(self.actual_goal_reached, new_goal_reached)
        return self.actual_goal_reached
    
    def move_condition(self, new_positions):
        in_bounds = self.position_in_bounds(new_positions)
        nr_active = new_positions.shape[0]
        return in_bounds.unsqueeze(1).expand(nr_active, ENV_2D), (self.bool_zeros(nr_active), self.bool_zeros(nr_active))
    ...
    def reset(self):
        ...
        self.agent_costs[:] = 0 # <-- 新增: 重置成本记录
        random.shuffle(self.occupiable_locations)
        if self.params.get(REAL_TIME_LOG, False):
            print(f"Number of occupiable locations: {len(self.occupiable_locations)}")  
        ...
        for a in range(self.nr_agents):
            ...
            if self.init_goal_radius is not None and self.init_goal_radius < max(self.rows, self.columns):
                ...
                if len(goal_candidates) == 0:
                    # 处理 goal_candidates 为空的情况
                    if self.init_goal_radius < max(self.rows, self.columns):
                        self.init_goal_radius = max(self.rows, self.columns)  # 增大 init_goal_radius
                        goal_candidates = self.get_neighbor_positions((x, y), self.init_goal_radius)
    ...
    def set_collision_weight(self, weight):
        self.dynamic_collision_weight = torch.tensor(weight)
